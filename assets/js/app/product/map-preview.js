...
//   let mapContent = document.querySelector('.map-content-scaled');
...
!(function () {
  ...
  var ANIMDISTANCE = 0.035,  // Rotation animation threshold, ~2deg in radians
    ...
    ANIMINTERVAL_P = 1, // Projection duration in ms
    ANIMINTERVAL_Z = 1, // Zoom duration scale in ms
    zoomextent = 10,       // Default maximum extent of zoom (max/min)
    zoomlevel = 1.018;      // Default zoom level, 1 = 100%
  ...
  var settings = {
    ...
    settimezone: false,  // Automatcally set time zone when geolocation changes
    ...
    planets: {  //Show planet locations, if date-time is set
      ...
      symbolStyle: { fill: "yellow", opacity: 1, font: "bold 17px 'Lucida Sans Unicode', Consolas, sans-serif", align: "center", baseline: "middle" },
    ...
    var draw_symbol = {
      ...
      "crescent": function (ctx) {
        var s = Math.sqrt(size()),
          ...
          darkFill = ph < 0.157 ? "transparent" : "transparent";
        ctx.save();
  ...
  */
  ...
  let errorCount = 0;
  window.saveSVG = function (mapColor, generatePDF) {
    ...
    console.log(document.querySelector('#d3-celestial-svg'));
      ...
      q.defer(function (callback) {
        d3.json(path + cfg.dsos.data, function (error, json) {
          ...
          objects.selectAll(".dsos")
            ...
            .style({
              "fill": function (d) {
                if (has(cfg.dsos.symbols[d.properties.type], "stroke")) return "none";
              ...
              "stroke-width": function (d) {
                if (!has(cfg.dsos.symbols[d.properties.type], "width")) return 1;
          ...
          if (cfg.dsos.names) {
            objects.selectAll(".dsonames")
              ...
              .attr({ dy: "-.5em", dx: ".35em" })
              .style({
                "fill": function (d) { return cfg.dsos.colors ? cfg.dsos.symbols[d.properties.type].fill : cfg.dsos.style.fill; },
                "fill-opacity": cfg.dsos.style.opacity,
        ...
        if (jlun.features.length > 0) {
          if (cfg.planets.symbolType === "letter") {
            ...
            planets.selectAll(".dmoon")
              ...
              .enter().append("path")
              .attr("class", "darkluna")
              .style("fill", function (d) { return d.properties.phase < 0.157 ? mapColor : mapColor; })
              .style("stroke", "white")
    ...
    function moonSymbol(p, r) {
      var size = r ? r * r : 121;
      return d3.svg.customSymbol().type("crescent").size(size + 100).ratio(p.age)();
    }
    ...
    q.await(function (error) {
      ...
      var svg = d3.select("#d3-celestial-svg svg")
        .attr("title", "Mapa das Estrelas")
        .attr("version", 1.1)
        .attr("xmlns", "http://www.w3.org/2000/svg");

      var blob = new Blob([svg.node().outerHTML], { type: "image/svg+xml;charset=utf-8" });

      console.log(document.querySelector("#d3-celestial-svg svg"));
      ...
      let randomStr = makeid(5);
      let svgContent = svg.node().outerHTML;
      let photoName = `mapa-${randomStr}`;

      if (svgContent.includes('width="750"')) {
        if (errorCount >= 1) {
          alert('502: Connection Error');
          location.reload();
          return;
        }
        errorCount++;
        saveSVG(mapColor, generatePDF);
        return;
      }

      svgContent = svgContent.replace(/stroke-dasharray: none;/g, '').replace(/font: 10px sans-serif;/g, 'font-size:5px; font-family: sans-serif;').replace(/ /g, ' ').replace(/font-size: 10px/g, 'font-size: 5px').replace(/font: 6px sans-serif;/g, 'font-size:5px; font-family: sans-serif;').replace(/font-size: 6px/g, 'font-size: 5px')

      if (svgContent.includes('star')) {
        if (document.querySelector('.wc-pao-addon-nome-das-constelacoes input').value === 'Sim' && !svgContent.includes('constname')) {
          alert('Connection lost. Try again');
          location.href = '/produto/meu-mapa/';
          return;
        }
        handleMapLoading(80);
        ...
        jQuery.ajax({
          ...
          let linkFormated;
          let svgInput = document.querySelector('.wc-pao-addon-link-do-svg input');
          let imagePath = `${location.origin}/photos/${photoName}.svg`;
          let pdfPath = document.querySelector('.wc-pao-addon-link-do-pdf input');
          let fullLink = `${pdfPath.value}&svg=${photoName}`;
          ...
          pdfPath.value = fullLink;
          svgInput.value = imagePath;
          ...
          if (generatePDF) {
            ...
            // window.open(
            //   `${location.origin}${linkFormated}&order_id=`,
            //   '_blank'
            // );
            // location.reload();
