...
!(function () {
  ...
  */
  ...
  let errorCount = 0;
  window.saveSVG = function (mapColor = '#000', generatePDF) {
    ...
    q.await(function (error) {
      ...
      if (svgContent.includes('star')) {
        ...
        var bottleCanvas = document.querySelector('#stars-preview canvas');

        var file = bottleCanvas.toDataURL("image/png");

        let photoId = makeid(5);
        let photoName = `pe-${photoId}`;
        ...
        jQuery.ajax({
          ...
          if (generatePDF) {
            ...
            var a = d3.select("body").append("a").node(),
              canvas = document.querySelector("#stars-preview canvas");
            a.download = 'pingente-das-estrelas.png';
            a.rel = "noopener";
            a.href = canvas.toDataURL('image/png').replace('image/png', 'image/octet-stream');
            a.click();
            d3.select(a).remove();
            ...
            if (document.querySelector('#pendant-chain').value !== 'veneziana') {
              handleMapLoading(100);
            ...
            jQuery
              .get('/wp-admin/admin-ajax.php', chainData)
        ...
        location.href = '/produto/pingente-das-estrelas/';
      }
    });
